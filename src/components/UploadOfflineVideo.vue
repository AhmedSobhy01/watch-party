...
const validateVideoLength = () => {
    ...
    video.onloadedmetadata = () => {
        if (Math.round(video.duration) == props.videoLength) {
            ...
            errorMessage.value = `Video length must be ${props.videoLength} seconds.`;
        }
    };
};
</script>
...
<template>
    <div class="flex flex-col items-center justify-center max-w-sm mx-auto h-screen gap-5">
        <input type="file" ref="videoFile" @change="validateVideoLength" accept="video/*" class="border p-3" />
